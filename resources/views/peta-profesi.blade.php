...
<head>
    ...
    <title>ShiroAsesmen - Peta Profesi</title>
...
<body class="bg-background text-on-background font-body-md selection:bg-primary-container selection:text-on-primary-container min-h-screen flex">
    ...
    <nav id="sidebar" class="hidden lg:flex h-screen w-64 fixed left-0 top-0 bg-[#800000] z-50 flex-col py-6 text-white shadow-xl overflow-hidden no-print">
        ...
        <div class="flex-1 flex flex-col gap-1 px-4">
            ...
            <!-- Active Tab -->
            <a class="flex items-center gap-3 px-4 py-3 text-[#FFD700] font-bold bg-white/10 border-r-4 border-[#FFD700] rounded-l-lg transition-colors duration-200" href="{{ url('/peta-profesi') }}">
    ...
    <div class="flex-1 lg:ml-64 flex flex-col min-h-screen relative">
            ...
            <div class="flex items-center justify-between px-4 md:px-8 py-3 max-w-[1280px] mx-auto">
                <div class="flex items-center gap-4">
                    ...
                    <div class="flex flex-col ml-2 md:ml-0">
                        ...
                        <span class="text-lg font-bold text-primary font-headline-md text-headline-md">Peta Profesi</span>
                    </div>
                </div>
        ...
        <main class="flex-1 p-8 max-w-[1280px] w-full mx-auto space-y-12">
            ...
            <div class="text-[10px] font-bold tracking-widest uppercase text-on-surface-variant mb-[-1rem]">
                SHIROASESMEN &bull; HASIL ASESMEN &bull; <span class="text-primary">PETA PROFESI</span>
            </div>

            <!-- Section 1: Ringkasan Arah Karir -->
            <section class="mb-12 relative rounded-2xl overflow-hidden shadow-lg border border-surface-variant bg-[#800000]">
                ...
                <div class="relative z-10 p-10 flex flex-col md:flex-row items-center justify-between gap-6">
                    <div class="w-full md:w-2/3">
                        <div class="inline-flex items-center gap-2 px-3 py-1 bg-[#FFD700]/20 text-[#FFD700] border border-[#FFD700]/30 rounded-full text-xs font-bold tracking-wide mb-4">
                            <span class="material-symbols-outlined text-[14px]">psychology</span>
                            ANALISIS KECOCOKAN KARIR
                        </div>
                        <h2 class="text-3xl font-black text-white italic mb-3 tracking-tight">{{ strtoupper(isset($sortedTopN[0]) ? ($sortedTopN[0]['name'] ?? 'Pilihan Karir Utama') : 'Pilihan Karir Utama') }}</h2>
                        <p class="text-white/90 font-body-md leading-relaxed text-sm md:text-base">
                            Berdasarkan profil Anda, perpaduan minat <strong>{{ collect($result->input_riasec ?? [])->sortDesc()->take(1)->keys()->map(fn($k) => $riasecLabels[$k] ?? ucfirst($k))->first() }}</strong> yang dominan, 
                            kepribadian <strong>{{ collect($result->input_trait ?? [])->sortDesc()->take(1)->keys()->map(fn($k) => $traitLabels[$k] ?? ucfirst($k))->first() }}</strong> yang menonjol, 
                            mengarahkan Anda pada karir di mana Anda dapat mengoptimalkan potensi terbaik Anda.
                        </p>
                    </div>
                    <div class="hidden md:flex w-full md:w-1/3 justify-end">
                        <div class="bg-white/10 backdrop-blur-md border border-white/20 p-5 rounded-xl w-full max-w-[250px]">
                            <h4 class="text-white text-sm font-bold mb-3 border-b border-white/20 pb-2">Kekuatan Utama</h4>
                            <ul class="space-y-2 text-white/90 text-xs">
                                <li class="flex items-center gap-2"><span class="material-symbols-outlined text-[#FFD700] text-sm">check_circle</span> Solusi Praktis (R)</li>
                                <li class="flex items-center gap-2"><span class="material-symbols-outlined text-[#FFD700] text-sm">check_circle</span> Akurasi & Keteraturan (C)</li>
                                <li class="flex items-center gap-2"><span class="material-symbols-outlined text-[#FFD700] text-sm">check_circle</span> Fokus Lingkungan (Tertib)</li>
                            </ul>
                        </div>
                    </div>
                </div>
            ...
            <!-- Section 2: Daftar Rekomendasi Profesi Berjenjang -->
            <section class="space-y-8">
                <div class="flex items-center gap-3 border-b border-surface-variant pb-4">
                    ...
                    <div>
                        <h3 class="text-2xl font-bold text-on-surface">Rekomendasi Profesi</h3>
                        <p class="text-sm text-on-surface-variant">Hierarki karir berdasarkan integrasi 4 dimensi profil Anda.</p>
                    </div>
                </div>

                <div class="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
                    @foreach(collect($sortedTopN)->take(3) as $index => $prof)
                    <!-- Profesi Recommendation {{ $index + 1 }} -->
                    <div class="bg-surface-container-lowest border {{ $index === 0 ? 'border-[#FFD700]/50 shadow-md' : 'border-outline-variant' }} rounded-2xl overflow-hidden shadow-sm hover:shadow-md transition-all flex flex-col relative">
                        @if($index === 0)
                            <div class="absolute top-0 right-0 bg-[#FFD700] text-[#800000] text-[10px] font-black px-3 py-1 rounded-bl-lg tracking-wider">DOMINAN</div>
                        @elseif($index === 1)
                            <div class="absolute top-0 right-0 bg-surface-variant text-on-surface-variant text-[10px] font-black px-3 py-1 rounded-bl-lg tracking-wider">POTENSIAL</div>
                        @else
                            <div class="absolute top-0 right-0 bg-surface-dim text-on-surface text-[10px] font-black px-3 py-1 rounded-bl-lg tracking-wider">ALTERNATIF</div>
                        @endif
                        
                        <div class="p-6 border-b border-surface-variant bg-surface-container-low flex items-center gap-4">
                            <div class="w-12 h-12 bg-white rounded-xl shadow-sm border border-surface-variant flex items-center justify-center shrink-0">
                                <span class="material-symbols-outlined text-[#800000] text-2xl">
                                    @php
                                        $icons = ['Software' => 'code_blocks', 'Network' => 'dns', 'Data' => 'database', 'Security' => 'security', 'Manager' => 'leaderboard'];
                                        $icon = 'work';
                                        foreach($icons as $key => $val) {
                                            if(str_contains($prof['name'], $key)) { $icon = $val; break; }
                                        }
                                    @endphp
                                    {{ $icon }}
                                </span>
                            ...
                            <div>
                                <h4 class="font-bold text-on-surface text-lg leading-tight">{{ $prof['name'] }}</h4>
                                <span class="text-xs text-on-surface-variant">{{ $prof['category'] ?? 'Bidang Profesional' }}</span>
                            </div>
                        </div>
                        <div class="p-6 flex-1 flex flex-col gap-4">
                            <div class="bg-primary/5 rounded-xl p-4 border border-primary/10">
                                <h5 class="text-xs font-bold text-primary mb-2 uppercase tracking-wide">Analisis Kecocokan ({{ number_format($prof['score'] ?? 0, 1) }}%)</h5>
                                <p class="text-xs text-on-surface-variant leading-relaxed">
                                    Profesi ini sangat sesuai dengan profil <strong>{{ collect($result->input_riasec ?? [])->sortDesc()->take(1)->keys()->first() ?? 'Dominan' }}</strong> Anda dan didukung oleh tingkat <strong>{{ collect($result->input_trait ?? [])->sortDesc()->take(1)->keys()->first() ?? 'Karakter' }}</strong> yang Anda miliki.
                                </p>
                            </div>
                            
                            <div class="space-y-3">
                                <div>
                                    <h5 class="text-xs font-bold text-on-surface-variant mb-1 flex items-center gap-1">
                                        <span class="material-symbols-outlined text-[14px]">school</span> Kompetensi Utama
                                    </h5>
                                    <p class="text-[12px] text-on-surface-variant/80 italic leading-relaxed">
                                        Membutuhkan pemahaman mendalam tentang prinsip {{ strtolower($prof['category'] ?? 'bidang terkait') }} dan kemampuan problem solving yang kuat.
                                    </p>
                                </div>
                                <div>
                                    <h5 class="text-xs font-bold text-on-surface-variant mb-1 flex items-center gap-1">
                                        <span class="material-symbols-outlined text-[14px]">payments</span> Estimasi Gaji
                                    </h5>
                                    @php $matchPct = $prof['score'] ?? 0; @endphp
                                    <p class="text-sm font-bold text-[#800000]">Rp {{ number_format(max(5, (100 - $matchPct) / 2) ?? 0, 0) }}.000.000 - Rp {{ number_format(max(10, (200 - $matchPct) / 3) ?? 0, 0) }}.000.000</p>
                                </div>
                            </div>
                            
                            <div class="mt-auto border-t border-surface-variant pt-3">
                                <h5 class="text-xs font-bold text-on-surface mb-2 flex items-center gap-1">
                                    <span class="material-symbols-outlined text-[14px] text-secondary">trending_up</span> Prospek Karir
                                </h5>
                                <div class="flex flex-wrap gap-1 text-[10px] font-bold">
                                    <span class="bg-surface-variant text-on-surface-variant px-2 py-1 rounded">Junior</span>
                                    <span class="material-symbols-outlined text-[12px] text-surface-variant self-center">chevron_right</span>
                                    <span class="bg-surface-variant text-on-surface-variant px-2 py-1 rounded">Senior</span>
                                    <span class="material-symbols-outlined text-[12px] text-surface-variant self-center">chevron_right</span>
                                    <span class="bg-[#FFD700]/20 text-[#800000] px-2 py-1 rounded border border-[#FFD700]/50">Expert/Lead</span>
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <div class="mt-8 bg-surface-container-low rounded-2xl p-6 border border-surface-variant">
                    <h4 class="text-lg font-bold text-on-surface mb-3 flex items-center gap-2">
                        <span class="material-symbols-outlined text-primary">explore</span> Eksplorasi Alternatif Lainnya
                    </h4>
                    <p class="text-sm text-on-surface-variant mb-4">Jika ketiga jalur utama di atas tidak memungkinkan, berikut adalah opsi profesi potensial lainnya yang masih memiliki irisan kuat dengan profil hasil asesmen Anda:</p>
                    <div class="flex flex-wrap gap-3">
                        <div class="px-4 py-2 bg-white border border-surface-variant rounded-lg shadow-sm text-sm font-semibold hover:border-primary transition-colors cursor-pointer">
                        ...
                        <div class="px-4 py-2 bg-white border border-surface-variant rounded-lg shadow-sm text-sm font-semibold hover:border-primary transition-colors cursor-pointer">
                            Dosen Pengajar Pemrograman
                        </div>
                        <div class="px-4 py-2 bg-white border border-surface-variant rounded-lg shadow-sm text-sm font-semibold hover:border-primary transition-colors cursor-pointer">
                            Tim Ahli IT (Pemerintahan)
                        </div>
                        <div class="px-4 py-2 bg-white border border-surface-variant rounded-lg shadow-sm text-sm font-semibold hover:border-primary transition-colors cursor-pointer">
                            Software House Founder
                        </div>
                    </div>
                </div>
            </section>

            <!-- Section 3: Laboratorium Karir (Custom Check) -->
            <section class="mt-12 bg-primary-container text-on-primary-container rounded-3xl p-8 md:p-12 shadow-xl relative overflow-hidden">
                <!-- BG Pattern -->
                ...
                <div class="relative z-10 max-w-3xl mx-auto text-center mb-8">
                    <span class="material-symbols-outlined text-5xl mb-4 text-[#FFD700]">experiment</span>
                    <h3 class="text-2xl md:text-3xl font-black italic mb-3 text-white">Laboratorium Profesi</h3>
                    <p class="text-primary-fixed-dim text-sm md:text-base">
                        Punya profesi impian yang tidak ada di daftar rekomendasi? Masukkan nama profesi tersebut untuk melihat seberapa cocok dengan profil psikometrik Anda.
                    </p>
                </div>
                ...
                <div class="relative z-10 max-w-2xl mx-auto">
                    <div class="flex flex-col md:flex-row gap-3">
                        <div class="relative flex-1">
                            ...
                            <input type="text" id="customProfessionInput" class="w-full pl-12 pr-4 py-4 rounded-xl border-none focus:ring-4 focus:ring-[#FFD700]/50 text-on-surface text-lg shadow-inner" placeholder="Misal: Data Scientist, Koki, Akuntan...">
                        </div>
                        <button onclick="checkCustomProfession()" class="px-8 py-4 bg-[#FFD700] hover:bg-[#ffe16d] text-[#800000] font-bold rounded-xl transition-all shadow-md active:scale-95 whitespace-nowrap flex items-center justify-center gap-2">
                            <span class="material-symbols-outlined">analytics</span> Periksa
                        </button>
                    </div>
                </div>

                <!-- Hasil Dinamis akan muncul di sini (Replace mode) -->
                <div id="customProfessionResult" class="relative z-10 max-w-3xl mx-auto mt-8 hidden">
    ...
    <script>
        ...
        function checkCustomProfession() {
            const inputVal = document.getElementById('customProfessionInput').value.trim();
            const resultContainer = document.getElementById('customProfessionResult');
            
            if(!inputVal) {
                alert("Silakan masukkan nama profesi terlebih dahulu.");
                return;
            }
            ...
            resultContainer.innerHTML = `
                <div class="bg-white/10 backdrop-blur-md border border-white/20 rounded-2xl p-8 text-center animate-pulse">
                    <span class="material-symbols-outlined text-white text-4xl mb-2 animate-spin">refresh</span>
                    <p class="text-white font-medium">Menganalisis kecocokan profil dengan profesi ${inputVal}...</p>
                </div>
            `;

            // Simulasi hasil setelah 1.5 detik
            setTimeout(() => {
                // Layout hasil (menggantikan konten loading)
                resultContainer.innerHTML = `
                    <div class="bg-surface-container-lowest text-on-surface rounded-2xl p-6 md:p-8 shadow-2xl border border-outline-variant transform transition-all duration-500 scale-100 opacity-100">
                        <div class="flex justify-between items-start border-b border-surface-variant pb-4 mb-6">
                            <div>
                                <h4 class="text-2xl font-black text-primary capitalize">${inputVal}</h4>
                                <span class="text-xs font-bold text-on-surface-variant bg-surface-variant px-2 py-1 rounded">Hasil Analisis AI (Simulasi)</span>
                            </div>
                            <div class="text-right">
                                <div class="text-3xl font-black text-[#800000]">65<span class="text-lg text-on-surface-variant">%</span></div>
                                <span class="text-[10px] uppercase tracking-widest text-on-surface-variant">Tingkat Kecocokan</span>
                            </div>
                        </div>

                        <div class="space-y-6 text-left">
                            <div>
                                <h5 class="font-bold text-sm mb-2 flex items-center gap-2"><span class="material-symbols-outlined text-primary text-sm">psychology</span> Keselarasan Karakter</h5>
                                <p class="text-sm text-on-surface-variant leading-relaxed">
                                    Profesi <strong>${inputVal}</strong> menuntut keterampilan adaptasi dan pemecahan masalah praktis. Berdasarkan profil Anda (Realistic tinggi), profesi ini <strong>bisa dijalankan</strong>, namun Anda mungkin akan menggunakan pendekatan yang lebih sistematis dan prosedural (mengandalkan Conscientiousness Anda) alih-alih pendekatan intuitif murni.
                                </p>
                            </div>
                            
                            <div class="bg-error-container/20 rounded-xl p-4 border border-error/10">
                                <h5 class="font-bold text-sm mb-2 text-error flex items-center gap-2"><span class="material-symbols-outlined text-sm">warning</span> Catatan Konstruktif & Tantangan</h5>
                                <p class="text-sm text-on-surface-variant leading-relaxed">
                                    Tantangan utama Anda mungkin ada pada aspek rutinitas non-teknis atau komunikasi persuasif, tergantung pada spesifikasi tugas harian profesi ini. Anda mungkin perlu membangun sistem kerja pribadi yang terstruktur agar tidak cepat lelah (burnout) jika berhadapan dengan tugas administratif.
                                </p>
                            </div>

                            <div class="pt-4 border-t border-surface-variant text-center">
                                <button onclick="document.getElementById('customProfessionResult').classList.add('hidden'); document.getElementById('customProfessionInput').value = '';" class="text-sm font-bold text-on-surface-variant hover:text-primary transition-colors">Tutup Analisis</button>
                            </div>
                        </div>
                    </div>
                `;
